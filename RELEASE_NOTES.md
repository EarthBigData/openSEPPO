...
- GUNW (Geocoded Unwrapped interferogram, the L2 InSAR pair product) now has a converter in the same spirit as `seppo_nisar_gcov_convert`: spatial subsetting by `-srcwin`/`-projwin` (with `-projwin_srs` and the degree-against-projected-grid auto-detect), Cloud Optimized GeoTIFF / BigTIFF raster output per interferometric layer, a fully self-contained windowed HDF5 subset (`-of h5`), optional reprojection (`-t_srs`/`-tr`) and downscaling, `-lg` grid listing, and the shared Earthdata/S3/HTTPS auth, caching and parallel-read machinery. Implemented in a new `nisar_tools_gunw` module that reuses the geocoded helpers in `nisar_tools`; the GCOV/GSLC paths are untouched.
- GUNW is structurally unlike GCOV/GSLC: its grids are not flat under `frequency{X}`. Each frequency carries three sub-groups — `unwrappedInterferogram`, `pixelOffsets` and `wrappedInterferogram` — each an independent geocoded grid with its own coordinate axes and resolution (the wrapped grid is ~4× finer than the other two), and each with a per-polarisation subgroup. Because `frequency{X}/{group}/{pol}` carries its own `xCoordinates`/`yCoordinates`/`projection`, it behaves as a self-contained grid path and the shared subsetting helpers point straight at it.
- Raster output is single-grid, one layer group per run, selected with `--layer_group` (default `unwrappedInterferogram`) — mirroring GCOV's single-frequency rule, since the three grids are at different resolutions and cannot share one raster. `-vars` selects layers within the group; omitted, the group's default set is written (unwrapped: `unwrappedPhase`, `coherenceMagnitude`, `connectedComponents`). Continuous layers keep float32 with a NaN nodata and are resampled with `--resample` (bilinear default) on reprojection; integer layers (`connectedComponents` uint16, `mask` uint32, bit-encoded) keep their native dtype, carry the source `_FillValue`, use nearest resampling and nearest COG overviews so codes are never invented. The complex `wrappedInterferogram` layer is written as its phase (`angle`, radians, float32).
- The `-of h5` subset carries each selected sub-group (all three by default), each windowed on its own axes to the same map extent — so grids at different resolutions all cover the requested ground — plus the identification group (with a window-accurate `boundingPolygon` and `listOfFrequencies`), orbit and attitude (reference and secondary, verbatim), processingInformation, and the radarGrid geolocation cubes windowed on their own coarse grid rather than copied whole. The subsetting walk is a single recursive traversal that slices any dataset whose trailing dimensions match its group's own grid, so a plan for a parallel read can never drift from what is written.
- `-of h5` reads the windowed grid payload through the same parallel, chunk-coalescing reader as the metadata and the COG path, not serially through h5py one grid at a time. The grid arrays — above all the ~4×-finer complex `wrappedInterferogram` — dominate the transfer, so planning them into the same prefetch as the ~110 scattered metadata datasets and fetching them concurrently is what keeps the h5 subset from lagging far behind the raster output. Measured on a coseismic Venezuela subset (688×695 window, HTTPS from a laptop): 7m00s → 4m32s, output byte-identical. The plan now weights each dataset by its *windowed* transfer rather than its full on-disk size, which both fixes the reported prefetch volume and improves the reader's per-worker balancing — the wrapped grid reads as 4.4 GB on disk but only tens of MB through a small window. Over HTTPS the gain is bounded by the Earthdata egress throttle, which caps aggregate throughput regardless of worker count; in-region `s3://` bypasses it and is the larger win.
- `-groups` restricts which grid sub-groups `-of h5` carries. The 20 m complex `wrappedInterferogram` grid is the bulk of the payload (16× the pixels of the other two, 8-byte complex), and deformation work rarely needs it, so `-groups unwrappedInterferogram pixelOffsets` produces a self-contained subset a fraction of the size and time — the same coseismic window went 98.8 MB / 4m32s to 22.1 MB / 3m05s, orbit/attitude/metadata and the recomputed boundingPolygon unchanged. No effect on raster output, which is already single-grid via `--layer_group`.
- `--report` (**experimental**) adds a coseismic InSAR quick-look, rendered straight from the windowed unwrapped-phase and coherence read so it runs alongside any output format: wrapped fringes on a cyclic colormap, relative line-of-sight displacement in cm on the diverging `seismic` map (referenced to the high-coherence median, scaled by the granule's own wavelength via `centerFrequency`), coherence, and an info panel carrying the acquisition dates, temporal baseline, geometry and displacement statistics. `--epicenter LON LAT` marks an event location, `--report_format` selects PNG or PDF, and the figure is stamped `Produced with openSEPPO vX.Y.Z`. The panel layout, colormaps and referencing defaults are provisional and marked experimental on the figure, at runtime, and in the CLI help; `matplotlib` is now a `nisar` optional dependency.

- **Time-series VRT stacks over a batch (`--no_time_series` to disable).** `process_gunw_task` had accepted a `time_series_vrt` argument since the tool was written and never used it -- the generators were not even imported -- so a batch produced a per-pair multi-layer VRT and nothing tying the pairs together. A batch now also writes one VRT per layer with one band per interferometric pair, ordered by reference acquisition, each band described and tagged with that date, beside a `.dates` sidecar. Each band is an independent 12-day interferogram, not a cumulative displacement: summing a chain needs a reference point and unwrapping-error handling, which this converter deliberately leaves to the caller. The writer is now shared with the SME2 converter (`write_timeseries_vrts` in `nisar_tools`), so both products stack the same way and a date whose granule only partly covers the requested box sends the whole stack to the union writer. Verified on five chained pairs of track 162 ascending frame 007 either side of the 2026-06-24 Venezuela M7.2/M7.5: all five resolved the identical `3441x1814` window and every band is bit-for-bit the COG of its own pair. Two GUNW-specific fixes came with it -- the per-file result carried the *source* CRS beside the *output* transform and size, so a `-t_srs` stack would have been labelled with the wrong CRS; and a GUNW granule name carries two datetime pairs, so a stack name built the GCOV way left the first granule's secondary acquisition times stranded beside the series span. Both are corrected.
- A worked example page, [GUNW examples](nisar_gunw_convert_examples.md), covers search-to-interferogram and search-to-stack end to end over HTTPS.

**New tool: `seppo_nisar_sme2_convert` — NISAR SME2 (soil moisture, L3) subsetting and format conversion**
- SME2 (Soil Moisture Estimate, the L3 product derived from GCOV) now has a converter in the same spirit as `seppo_nisar_gcov_convert`: spatial subsetting by `-srcwin`/`-projwin` (with `-projwin_srs` and the degree-against-projected-grid auto-detect), Cloud Optimized GeoTIFF / BigTIFF raster output per layer, a fully self-contained windowed HDF5 subset (`-of h5`), optional reprojection (`-t_srs`/`-tr`) and downscaling, `-lg` grid listing, and the shared Earthdata/S3/HTTPS auth machinery. Implemented in a new `nisar_tools_sme2` module that reuses the geocoded helpers in `nisar_tools`; the GCOV/GSLC/GUNW paths are untouched.
- SME2 is the simplest of the geocoded products and the only one not on a UTM grid: every 2-D layer sits on one shared **EASE-Grid 2.0 Global (EPSG:6933)** grid at a 200 m posting, so a single window covers the whole granule. Because those coordinates are metres in the ±17 000 km range, a lon/lat `-projwin` given without `-projwin_srs` would snap to an edge pixel and write a silent 1x1 subset; the same auto-detect used by GCOV is applied to every window. Granules carry their `EASEGridRowIndex`/`EASEGridColumnIndex` window on the global grid and subsets keep it, so tiles from different dates and tracks align pixel-for-pixel with no resampling.
- Layers are discovered in each granule rather than hardcoded. SME2 moves layers between release tiers — `surfaceQualityFlag` sits under `grids/ancillaryData/` in BETA and directly under `grids/` in PROVISIONAL — and the `algorithmCandidates` set varies by granule (the sampled ones carry only DSG even where the run configuration requested DSG_TSR_PMI), so a hardcoded path table would silently drop layers. `--layer_group` selects `soilMoisture` (default), `algorithmCandidates` (`--algorithm`), `ancillaryData`, or `radarData` (`-f A|B`); `-lg` lists what a file actually holds. Float layers are written as float32 with NaN nodata; integer layers (quality flags, land cover, looks) keep their native dtype and `_FillValue` and use nearest resampling and overviews.
- `-of h5` carries every grid sub-group windowed on the shared grid — including the 1-D `latitude`/`longitude` and EASE index axes — plus the identification group with a window-accurate `boundingPolygon`, and the metadata groups verbatim (they carry no map grid). `-groups` restricts which sub-groups are carried. Verified dataset by dataset against the source window on a PROVISIONAL granule: 111 of 111 datasets present, 39 windowed and 71 verbatim, with no value and no attribute differences, and outputs from `s3://` identical to the same run against a local copy.
- Remote granules are read in place; there is no automatic caching. Measured from the granule's own chunk index, a 387x306 three-layer window costs 12 chunk reads and 2.77 MB, and a full-frame three-layer conversion 48 reads and 8.67 MB, against 90.2 MB to fetch the granule first — so caching costs more than it saves unless several groups are converted from the same file. `-cache` remains available for that case.
- **Time-series VRT stacks over a batch.** SME2 is the product time series are built from, and it was the only converter that did not stack one: a batch wrote a multi-layer VRT per date and nothing tying the dates together, while GCOV and GSLC have built time-series stacks all along. A batch now also writes one VRT per layer with one band per date in date order, each band described and tagged with its acquisition date, beside a `.dates` sidecar listing them -- the same arrangement and the same generators the GCOV stacks use. Because every SME2 layer is its own single-band COG, each band of the stack is band 1 of a different file. Repeat passes of a frame resolve to the same window on the shared EASE-Grid, so the dates normally stack with no resampling at all; the geometries are compared rather than assumed, and a date whose granule only partly covers the requested box makes the whole stack fall back to the union writer, announced on stdout. Verified on five consecutive 12-day passes of one frame (cycles 004-008, track 042 descending, frame 069): all five resolved the identical window `col=723 row=654 w=387 h=301` and identical geotransform, and every band of the stack is bit-for-bit the COG of its own date. Verified on the union path too, by mixing two adjacent frames whose windows differ (483x1013 against 483x626): each band lands at the right offset in the union grid with its values intact. Integer layers keep their dtype and fill value in the stack. `--no_time_series` disables it; `-of h5` output is unaffected.
- **Granules in a batch are converted concurrently (`-j/--jobs`, default 4).** SME2 is the product people build time series from, and the per-granule cost is almost all fixed: opening the granule and reading the grid axes moves ~5 MB before a single pixel, against ~10-17 MB for a whole three-layer frame. That fixed cost is paid once per date, so dates overlap well even though there is little to overlap inside one. Concurrency has to be process-based -- h5py serialises every HDF5 call behind one global lock, so threads would take turns rather than overlap the network waits -- and each worker rebuilds its own filesystem from the auth configuration rather than inheriting a pickled one. Measured in-region on four full frames: COG 11.5s to 4.2s, and `-of h5` on two granules 12.7s to 8.4s. All twelve COGs and both HDF5 subsets are byte-identical to the serial run. `-j 1` restores one-at-a-time conversion, and a session with no importable `__main__` (a notebook) falls back to it automatically.
- **`--read_threads` now drives an actual read, and defaults to off.** It was advertised as parallel connections for reading HDF5 chunks, but nothing on the SME2 path ever used it: layers were read one at a time with plain h5py slices and the value was forwarded to GDAL as a warp thread count. It now prefetches every selected layer window through `parallel_read_datasets`, the same coalescing reader the GCOV and GSLC subsetters use -- not the band reader they use for imagery, which forces float32 and would destroy the native dtype of SME2's quality flags, land cover and look counts. The default is `1`, because the measurement does not support anything higher: an SME2 layer is 1665x2025 in 512x512 chunks, so a full frame is 16 chunks per layer, and spawning readers costs more than the round trips it saves -- in-region on a full three-layer frame, 2.5s serial against 3.5s with 8 workers over `s3://`, and 4.9s against 7.0s over HTTPS, with identical output either way. It is kept, and honest, for high-latency links where the arithmetic changes. With `-j > 1` the concurrency budget is spent across granules instead, and GDAL is given a share of the cores rather than all of them, so the two levels never oversubscribe.
- The `-of h5` writer still copies its ~111 datasets serially within a granule; batch concurrency covers the time-series case, and the same round-trip arithmetic applies to a single subset.
...
**`-of h5` no longer warns on every variable it writes**
- Each GCOV h5 subset printed `UserWarning: endian-ness of dtype and endian kwarg do not match` for the coordinate variables and again from the generic writer.  NISAR declares its arrays little-endian explicitly (`<f8`) and netCDF4 stores variables in its own byte order whatever it is handed, so passing the source dtype straight through asked for something the library was never going to honour.  It is now handed the same type in native order, which says what it was going to do anyway.
- Values, dtypes and stored byte order are unchanged -- the subset's arrays are still `<f8`/`<f4`, byte-identical to before, 317 datasets and the same file size -- and a run is now clean: zero warnings.  Compound, string and object dtypes are left alone, so the `filterwarnings` the compound path needed is the only one left.
