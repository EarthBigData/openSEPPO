...
- On a slow link neither request count nor coalescing is the lever that matters -- bytes are.  The same GCOV subset from a laptop over the public internet took 6m53s before any of this and 6m20s with coalescing but unaligned stripes, which fetched twice the bytes to save requests.  Where the transfer has to shrink, `-noanc` halves it (the two float ancillary grids are 51% of the wire bytes for this window) and `-vars` on a single covariance term halves it again.
- **The metadata prefetch reads its chunk ranges directly too.**  Its 225 datasets are small and scattered, which is the worst case for a page-aligned block cache: they sit roughly one per 4.2 MB block, so serving 8.3 MB of wanted data fetched 809 MB.  Taking their spans from the chunk index and serving them to h5py from memory -- the same machinery the payload uses, generalised past the 2-D case to any rank, chunked or contiguous -- brings that to 55 MB in 366 requests.  The datasets really are scattered, so the request count is what it is: relaxing the waste budget from 10% to 200% saves only 372 requests -> 313.  GSLC's prefetch takes the same path.
- Together the two changes take one 4124 x 4057 dual-pol subset from 1254 MB on the wire to 274 MB: payload 445 -> 219 MB, metadata 809 -> 55 MB.  In-region that is nearly invisible -- bytes are cheap there, and the timings above move by a second either way -- which is exactly why it went unnoticed.  It is the whole cost on a home link: the 6m53s laptop run was 1254 MB at 3.0 MB/s.
