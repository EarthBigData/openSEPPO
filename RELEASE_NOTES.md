...
- The `-of h5` subset carries *every* sub-group, each windowed on its own axes to the same map extent — so grids at different resolutions all cover the requested ground — plus the identification group (with a window-accurate `boundingPolygon` and `listOfFrequencies`), orbit and attitude (reference and secondary, verbatim), processingInformation, and the radarGrid geolocation cubes windowed on their own coarse grid rather than copied whole. Scattered metadata datasets are prefetched with the same parallel reader the GSLC subsetter uses. The subsetting walk is a single recursive traversal that slices any dataset whose trailing dimensions match its group's own grid, so a plan for a parallel read can never drift from what is written.
