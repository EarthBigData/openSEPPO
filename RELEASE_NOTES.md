...
**GCOV subsetting speed: parallel reads over HTTPS too, and a parallel metadata read for `-of h5`**
...
- Measured in-region on a 4124 x 4057 dual-pol frequency-A window.  COG: local 3.5 s unchanged, `s3://` 8.2 s -> 5.2 s, HTTPS 19.8 s -> 11.9 s (the read itself 14.7 s -> 6.4 s).  `-of h5`: local 10.1 s unchanged, `s3://` 54 s -> 32 s, HTTPS 131 s -> 66 s.
