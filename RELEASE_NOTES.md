...
- `-of h5` read its payload one hyperslab at a time.  A 4124 x 4057 window spans 81 of the granule's 512x512 chunks, and the covariance terms and the four ancillary grids each span the same 81, so a subset issued 486 chunk requests back to back -- which is why it was slow in a way no amount of metadata prefetching could fix.  Those reads now go through the coalescing parallel reader written for the RSLC subsetter: chunk ranges taken from the granule's own index, **486 requests collapsed to 135**, spread over `--read_threads` workers.  That reader now lives in `nisar_tools` and both subsetters use it.
- Measured in-region on a 4124 x 4057 dual-pol frequency-A window.  COG: local 3.5 s unchanged, `s3://` 8.2 s -> 5.2 s, HTTPS 19.8 s -> 11.9 s (the read itself 14.7 s -> 6.4 s).  `-of h5`: local 10.1 s unchanged (never read-bound), `s3://` 54 s -> 21 s, HTTPS 131 s -> 43 s.  Request count, not bandwidth, is what a high-latency link pays for: the same subset from a laptop over the public internet took 6m53s before this change, almost exactly the 486 serial requests at that link's ~0.85 s per DAAC range request.
...
- Output is unchanged: the COGs are byte-identical and all 317 datasets of the h5 subset match the serial writer's, from all three transports.  (The h5 file's md5 is not stable run to run -- netCDF/h5repack packs it differently each time -- so equality here was checked dataset by dataset, not by digest.)
